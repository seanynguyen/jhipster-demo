...
import com.mycompany.myapp.web.rest.dto.RequestMessageDTO;
...
public class UserResource {
     ...
     */
    @RequestMapping(value = "/users/friends",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @Transactional(readOnly = true)
    public ResponseEntity<List<ManagedUserDTO>> getLoggedInUsers() {
        List<User> users = userRepository.findOneByLogin(SecurityUtils.getCurrentUserLogin()).get().getFriends();
        return ResponseEntity.ok()
                .body(users.stream()
    ...
    /**
     * POST  /users/sendMessage -> send messages.
     */
    @RequestMapping(value = "/users/sendQueryMessage",
            method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<String>> sendQueryMessage(@RequestBody RequestMessageDTO requestMessageDTO) {
        return ResponseEntity.ok().body(userService.sendMessage(requestMessageDTO.getContent()
                , requestMessageDTO.isPublic()));
...
//     * POST  /users/sendMessage -> send messages.
//     */
//    @RequestMapping(value = "/users/getMessages",
//                    method = RequestMethod.GET,
//                    produces = MediaType.APPLICATION_JSON_VALUE)
//    public ResponseEntity<Set<Message>> getCurrentUserMessages() {
//        User currentUser =  userRepository.findOneByLogin(SecurityUtils.getCurrentUserLogin()).get();
//        return ResponseEntity.ok().body(currentUser.getMessages());
//    }

//    /**
//     * GET  /users/testSessionRegistry -> test Session Registry.
//     */
//    @RequestMapping(value = "/users/testSessionRegistry",
//            method = RequestMethod.GET,
//            produces = MediaType.APPLICATION_JSON_VALUE)
//    @Timed
//    @Transactional(readOnly = true)
//    public ResponseEntity<List<ManagedUserDTO>> testSessionRegistry() {
//
//        return ResponseEntity.ok()
//                .body(sessionRegistry.getAllPrincipals().stream()
//                        .map(principal -> new ManagedUserDTO((User) principal))
//                        .collect(Collectors.toList()));
//    }
    ...
    public void sendMessage(Boolean isPublished) {
        if (isPublished) {
